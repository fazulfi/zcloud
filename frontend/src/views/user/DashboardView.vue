  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <template v-else-if="stats">
        <ModelCatalogSection :model-balances="modelBalances" />
        <UserDashboardStats :stats="stats" :balance="user?.balance || 0" :is-simple="authStore.isSimpleMode" :model-balances="modelBalances" :platform-quotas="platformQuotas" />
...
import UserDashboardStats from '@/components/user/dashboard/UserDashboardStats.vue'; import ModelCatalogSection from '@/components/user/dashboard/ModelCatalogSection.vue'; import UserDashboardCharts from '@/components/user/dashboard/UserDashboardCharts.vue'
...
const refreshAll = () => { loadStats(); loadCharts(); loadRecent(); if (showPlatformBreakdown.value) loadPlatformQuotas(); loadModelBalances() }

onMounted(async () => { await appStore.fetchPublicSettings(); refreshAll() })
</script>
