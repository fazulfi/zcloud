  ...
  <header class="glass sticky top-0 z-30 border-b border-gray-200/50 dark:border-dark-700/50">
    <div class="flex h-16 items-center justify-between gap-2 px-2 sm:px-4 md:px-6">
      ...
      <div class="flex min-w-0 items-center gap-1 sm:gap-3">
        ...
        <!-- Token Balance (total tokens across models) -->
        <div
          v-if="user && tokenTotal !== null"
        ...
        >
          ...
          <span class="text-sm font-semibold text-primary-700 dark:text-primary-300">
            {{ formatTokenBalance(tokenTotal) }}
          </span>
          <span class="hidden text-xs font-medium text-primary-500 dark:text-primary-400 lg:inline">
            {{ t('common.tokens') }}
          </span>
        </div>
        ...
        <div v-if="user" class="relative" ref="dropdownRef">
          ...
          <transition name="dropdown">
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div v-if="tokenTotal !== null" class="border-b border-gray-100 px-4 py-2 dark:border-dark-700 sm:hidden">
                <div class="text-xs text-gray-500 dark:text-dark-400">
                  {{ t('common.tokens') }}
                </div>
...
import { getModelBalances } from '@/api/usage'
...
const tokenTotal = ref<number | null>(null)

const loadTokenTotal = async (): Promise<void> => {
  if (!user.value) return
  try {
    const balances = await getModelBalances()
    tokenTotal.value = balances.reduce((sum, b) => sum + Number(b.balance || 0), 0)
  } catch {
