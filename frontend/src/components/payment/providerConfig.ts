...
export const METHOD_ORDER = ['alipay', 'alipay_direct', 'wxpay', 'wxpay_direct', 'stripe', 'airwallex', 'qris'] as const
...
export const WEBHOOK_PATHS: Record<string, string> = {
  ...
  gomerch: '/api/v1/payment/webhook/gomerch',
}
...
export const PROVIDER_CALLBACK_PATHS: Record<string, CallbackPaths> = {
  ...
  gomerch: { notifyUrl: WEBHOOK_PATHS.gomerch },
}
...
export const PROVIDER_CONFIG_FIELDS: Record<string, ConfigFieldDef[]> = {
  ...
  gomerch: [
    ...
    { key: 'webhookSecret', label: 'Webhook Secret', sensitive: true, optional: true },
    { key: 'apiBase', label: 'API Base URL', sensitive: false, defaultValue: 'https://v1-api.pay-gomerch.web.id' },
    { key: 'currency', label: 'Currency', sensitive: false, defaultValue: 'IDR', options: PAYMENT_CURRENCY_OPTIONS },
    { key: 'idrRate', label: 'IDR Rate (IDR per USD)', sensitive: false, defaultValue: '16000' },
  ],
}
