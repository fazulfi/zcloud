...
describe('PlazaModelCard', () => {
  it('renders model name and buy button navigation', async () => {
    ...
    expect(routerPush).toHaveBeenCalledWith('/model-plaza/model/claude-sonnet')
  })

  it('preserves token price and cache formatting', () => {
  ...
  it('applies custom rates while keeping official prices unchanged', () => {
  ...
  it('renders per-request and per-image pricing chips', () => {
    const model = tokenModel({ pricing: { billing_mode: 'image', input_price: null, output_price: null, cache_write_price: null, cache_read_price: null, image_input_price: null, image_output_price: 3e-5, per_request_price: null, intervals: [{ min_tokens: 0, max_tokens: null, tier_label: '1K', input_price: null, output_price: null, cache_write_price: null, cache_read_price: null, per_request_price: 0.01 }] }, official_pricing: null })
  ...
  it('renders tier labels and time-period row pricing', () => {
    ...
    expect(period.text()).toContain('$1.20')
