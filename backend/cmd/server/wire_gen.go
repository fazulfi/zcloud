...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	supplierPricingRepository := repository.NewSupplierPricingRepository(client)
	supplierPricingResolver := service.NewSupplierPricingResolver(supplierPricingRepository)
	reservationRepository := repository.NewUsageReservationRepository(client, db)
	usageModelSnapshotRepository := repository.NewUsageModelSnapshotRepository(client)
	modelBalanceRepository := repository.NewModelBalanceRepository(client)
	gatewayService := service.NewGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, identityService, httpUpstream, deferredService, claudeTokenProvider, sessionLimitCache, rpmCache, digestSessionStore, settingService, tlsFingerprintProfileService, channelService, modelPricingResolver, compositeRouteResolver, balanceNotifyService, serviceUserPlatformQuotaRepository, supplierPricingResolver, reservationRepository, usageModelSnapshotRepository, modelBalanceRepository)
	...
	usageHandler := handler.NewUsageHandler(usageService, apiKeyService, opsService, settingService, modelBalanceRepository)
	redeemHandler := handler.NewRedeemHandler(redeemService, settingService)
	...
	adminOperationalRepository := repository.NewAdminOperationalRepository(db, client)
	operationalHandler := admin.NewOperationalHandler(adminOperationalRepository)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, modelBalanceRepository, notificationEmailService)
	...
	modelPlazaService := service.NewModelPlazaService(channelRepository, groupRepository, pricingService, billingService, modelPricingResolver, paymentConfigService)
	...
	accountDataHandler := handler.NewAccountDataHandler(userService, apiKeyService, subscriptionService, paymentService, client)
	supportContactHandler := handler.NewSupportContactHandler(emailService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, accountDataHandler, supportContactHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
