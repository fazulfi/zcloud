...
func (tx *Tx) init() {
	...
	tx.ApiKeyModelScope = NewApiKeyModelScopeClient(tx.config)
	...
	tx.ModelBalance = NewModelBalanceClient(tx.config)
	...
	tx.QrisPayment = NewQrisPaymentClient(tx.config)
	tx.RedeemCode = NewRedeemCodeClient(tx.config)
	tx.SecuritySecret = NewSecuritySecretClient(tx.config)
	tx.Setting = NewSettingClient(tx.config)
	tx.SubscriptionPlan = NewSubscriptionPlanClient(tx.config)
	tx.SupplierPricing = NewSupplierPricingClient(tx.config)
	tx.TLSFingerprintProfile = NewTLSFingerprintProfileClient(tx.config)
	tx.UsageCleanupTask = NewUsageCleanupTaskClient(tx.config)
	tx.UsageLog = NewUsageLogClient(tx.config)
	tx.UsageModelSnapshot = NewUsageModelSnapshotClient(tx.config)
