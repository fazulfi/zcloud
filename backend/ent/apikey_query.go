...
type APIKeyQuery struct {
	...
	withModelScopes *ApiKeyModelScopeQuery
...
// QueryModelScopes chains the current query on the "model_scopes" edge.
func (_q *APIKeyQuery) QueryModelScopes() *ApiKeyModelScopeQuery {
	query := (&ApiKeyModelScopeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withModelScopes: _q.withModelScopes.Clone(),
...
// WithModelScopes tells the query-builder to eager-load the nodes that are connected to
// the "model_scopes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithModelScopes(opts ...func(*ApiKeyModelScopeQuery)) *APIKeyQuery {
	query := (&ApiKeyModelScopeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withModelScopes = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withModelScopes; query != nil {
		if err := _q.loadModelScopes(ctx, query, nodes,
			func(n *APIKey) { n.Edges.ModelScopes = []*ApiKeyModelScope{} },
			func(n *APIKey, e *ApiKeyModelScope) { n.Edges.ModelScopes = append(n.Edges.ModelScopes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadModelScopes(ctx context.Context, query *ApiKeyModelScopeQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *ApiKeyModelScope)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikeymodelscope.FieldAPIKeyID)
	}
	query.Where(predicate.ApiKeyModelScope(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.ModelScopesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
